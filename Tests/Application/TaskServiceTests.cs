...
namespace TaskManagement.Tests.Application
{
    public class TaskServiceTests
    {
        ...
        public void CreateTask_ShouldThrowException_WhenTooManyTasksInProject()
        {
            ...
            _taskRepositoryMock.Setup(repo => repo.GetTasksByProjectId(It.IsAny<Guid>())).Returns(Enumerable.Range(1, 21).Select(x => new Domain.Entities.Task(
                "Existing Task",
                "Description",
                DateTime.Now,
                TaskPriority.Baixa,
        ...
        public void UpdateTask_ShouldUpdateTaskStatus_WhenValidTaskIdAndStatusAreProvided()
        {
            ...
            _taskRepositoryMock.Setup(repo => repo.UpdateTask(It.IsAny<Domain.Entities.Task>())).Verifiable();

            var newStatus = TaskStatus.EmAndamento;

            // Act
            _taskService.UpdateTask(taskId, newStatus, taskDto); // Passando o taskDto completo

            // Assert
            Assert.Equal(newStatus, task.Status);
            _taskRepositoryMock.Verify(repo => repo.UpdateTask(It.IsAny<Domain.Entities.Task>()), Times.Once);
        }
        ...
        public void UpdateTask_ShouldThrowException_WhenTaskNotFound()
        {
            ...
            _taskRepositoryMock.Setup(repo => repo.GetTaskById(taskId)).Returns((Domain.Entities.Task)null);

            // Act & Assert
            var exception = Assert.Throws<Exception>(() => _taskService.UpdateTask(taskId, TaskStatus.Completada, taskDto));
